...
class Ajax extends Backend
{
    ...
    public function upload()
    {
        ...
        if ($splInfo) {
            ...
            $route = $_SERVER['DOCUMENT_ROOT'].$uploadDir . $splInfo->getSaveName();
//            $route = $_SERVER['DOCUMENT_ROOT'].'/fastadmin/public'.$uploadDir . $splInfo->getSaveName();
            $filePaths = $this->upload_oss($route);

            $ids = Db::name("attachment")->where(['url'=>$uploadDir . $splInfo->getSaveName()])->column("id");
    ...
    function del($ids = ""){
        if ($ids) {
            ...
            $attachmentlist = Db::name("attachment")->where('id', 'in', $ids)->select();
            foreach ($attachmentlist as $attachment) {
                \think\Hook::listen("upload_delete", $attachment);
                Db::name("attachment")->where(['id'=>$attachment['id']])->delete();
            }
        }
    }
    ...
    function upload_oss($route, $folder = "image/") {
        ...
        vendor('OSS.Core.OssException');
        vendor('OSS.Http.RequestCore');
        vendor('OSS.Http.ResponseCore');
        vendor('OSS.Http.RequestCore_Exception');
        ...
        return "https://yuntingdiancan.oss-cn-hangzhou.aliyuncs.com/" . $object;
    }
